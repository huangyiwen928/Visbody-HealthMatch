...
**问卷设计器**是一个可视化工具，允许运营人员快速创建、发布问卷。其核心价值在于**“问卷标签化”**——将用户的每一个回答，自动转化为元数据字典中的一个**“问卷标签”**，直接赋能给推荐算法。

### 1.2 核心用户故事 (User Story)
> **作为** 运营管理员，
> **我希望** 能通过简单的“添加题目-输入选项”的方式配置问卷，
> **并且** 不需要手动维护复杂的标签代码，系统能根据我的选项内容自动生成**问卷标签**，
...
*   **本产品模式**：配置选项（如“侧卧”） -> 系统自动生成“侧卧”**问卷标签** -> 用户选择即被打标。

**数据流向**：
1.  **定义**：运营在问卷中定义 `选项 = 问卷标签`。
2.  **采集**：C端用户选择选项。
3.  **沉淀**：系统将标签沉淀到【元数据字典】的**“问卷标签”**库中。
4.  **应用**：【推荐策略】读取这些标签进行匹配。
...
*   **选项配置（智能填充）**：
    ...
    *   **修正**：运营可点击标签输入框进行修改（如改为“咖啡依赖”）。
    *   **断开同步**：一旦运营**手动修改**了标签内容，该字段与选项内容的**自动同步关系即刻解除**，后续选项文案的变动不再影响该标签。
    *   **校验**：
        *   同一个问卷内，标签名称不允许重复。
        *   标签名称长度限制：**1-15个字符**。
        *   格式限制：仅支持中文、英文、数字，不支持特殊符号。
...
2.  **标签合法性检查**：
    *   所有选项必须配置标签名称（不能为空）。
    *   **格式规范**：标签长度不超过 15 字，且不能包含特殊字符。
    *   **唯一性**：同一问卷内，所有选项的标签名称**不可重复**。（例如：题目1有“失眠”选项，题目2就不能再有“失眠”选项，否则推荐系统无法区分来源）。
3.  **冲突检查**：
    *   系统内是否已存在同名且“已发布”的问卷。
    *   **标签全局冲突检查**：即使是已下架的问卷，重新上架时也需校验其包含的标签是否与当前线上其他问卷的标签冲突。
